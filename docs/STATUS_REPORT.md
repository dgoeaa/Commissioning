...
**Findings: 15 raised · 13 closed · 2 partial · 0 open.** *(F-020 closed 2026-08-31.)*
...
|---|---|---|---|
| **G-03** | 22 SAS signatures in tracked files | Ratchet blocks new ones in the application tree | **Rotate them in Power Automate.** Deleting files revoked nothing. **Corrected 6 Aug:** "reduced to 4 signatures in 2 files" counted the application tree only, which is the ratchet's scope and not rotation's. The whole-repository figure is **43 distinct signatures across 28 files** (read 55 until 2026-09-03, when the counters were normalised to the 43 characters that are the credential — see README), all under `docs/reference/foundational/`. See O-05 |
...
| **G-08** | Quality gate | Imports, secrets, governance, auth, smoke — 5 suites. **Rendered-appearance coverage exists and passes:** re-run 2026-08-31 against the container's pre-installed Chromium, **150 browser assertions, 0 failures** across all ten spec files — boot, all 29 routes, themes, portal, containment, scan intake, and `audit-remediation.spec.js`'s overflow and truncation checks at every width | **The `overrides` cascade debt stays unmeasured.** That is what is left of this finding: appearance is now asserted, but the cascade producing it is not. Note also that `npm test`'s final step still needs `DGO_CHROME_PATH` pointed at a browser in this container — the pinned Playwright build and the pre-installed one differ |

### Open — 0

| ID | Finding | Status |
|---|---|---|
| **F-020** | `canAccess()` grants more than the role table shows | ✅ **Closed 2026-08-31** — and it was worse than recorded below. See the closure note under the finding. |

**F-020 · The persona fallback overrides the role table.** `config/rbac.config.js` presents
`RoleRouteAccess` as the route matrix — `operator` and `director` are not granted `settings`
or `diagnostics` there. But `canAccess()` falls **through** to a persona check when the role
check fails, and the `registry` persona (mapped from both `director` and `operator`) is
allowed every route except `user-admin`. Both roles therefore reach Administration and System
Health, including the connection-address editor.

Measured, not inferred:

```
canAccess({role:'operator', persona:'registry'}, 'settings')    → true
canAccess({role:'operator', persona:'registry'}, 'diagnostics') → true
canAccess({role:'director', persona:'registry'}, 'settings')    → true
canAccess({role:'viewer',   persona:'general'},  'settings')    → false
```

This is distinct from the known client-side advisory limitation (§4): it is not tampering,
it is the configuration's own precedence granting access the role table appears to withhold.

**Closure, 2026-08-31 — the finding understated it.** The text below reports two roles reaching
two routes. Measured across the whole cross product of six roles and every route, **five of the
six roles were granted more than their own row allows**, and `RoleRouteAccess` was decorative for
everything but `systemAdmin`:

| Role | Routes in the table | Routes actually granted |
|---|---:|---:|
| `systemAdmin` | 26 | 26 |
| `userAdmin` | 5 | **26** |
| `viewer` | 5 | **21** |
...
`userAdmin` reached everything because its persona is `admin`, which returned `true` outright.
A route table read by a human and not by the code is worse than no table: it is a document
asserting a control that does not exist.

The fix is one line — a role now answers from its own row and only from its own row, with no
fall-through. The persona branch is kept for the one caller that has no role to offer: the R11.1
lineage shell passes a bare persona string, and that path is unchanged.

Why the existing tests passed while this stood: every one of them built its subject as
`{ role, status }` with **no persona**, and the fall-through only fired when a persona was
present. Every real caller has one — `normalizeUserRecord()` derives it from the role. So the
suite was testing a subject the application never constructs. The regression test added with the
fix asserts the property over the whole cross product instead of picking examples, because the
defect lived precisely in the routes nobody thought to write an example for.

It was found while assessing audit finding I-08's remaining recommendation — to *move* the
connection-address editor to a separate IT-only screen. **That relocation was declined on this
evidence:** Administration and System Health are reachable by exactly the same personas, so
moving the editor between them changes nothing about who can open it. The editor instead reads
as clearly technical in place, behind an IT-only disclosure with a danger treatment and plain
labels. Fixing access means fixing the precedence in `canAccess()`, which is RBAC policy and
was out of scope for the front-end redesign that surfaced it.
