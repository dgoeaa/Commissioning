...
- Expire and single-use the code. Cap attempts, then invalidate. The comparison is
  not constant-time — WDL has no bitwise operators, so `equals()` is what compares them; the protection is the lifecycle (short expiry, single use, attempt cap), per SC-003 / INT-005.
