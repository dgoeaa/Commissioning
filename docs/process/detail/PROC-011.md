...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-025` modules/fasttrack.js<br>`SRC-009` config/module-boundaries.config.js |
...
| Notifications issued | NOTIF-090 Escalation resolved<br>NOTIF-091 The registry could not be reached, so nothing was reloaded. Try again, or contact IT support if it keeps failing. |
| Downstream handoffs | — |
...
| --- | --- | --- | --- |
| SUBPROC-063 | Resolve the escalation | Reusable governed write | An operator activates the control bound to this action. |
