...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-027` modules/lookup.js<br>`SRC-009` config/module-boundaries.config.js |
...
| --- | --- | --- | --- |
| STEP-0038 | Mark the document | activities workspace | Manual — operator-initiated |
| STEP-0039 | Update the task | orchestrator workspace | Manual — operator-initiated |
| STEP-0040 | Raise a task from that email | single-assignment workspace | Manual — operator-initiated |
...
| Notifications issued | NOTIF-092 Records reloaded from the registry<br>NOTIF-093 The registry could not be reached — nothing was reloaded<br>NOTIF-094 Enter a number, reference or title first<br>NOTIF-095 The registry has been asked for this record — it joins the results when it comes back<br>NOTIF-096 The registry could not be reached, so only the records already on this device were searched<br>NOTIF-097 The mark is saved here — it goes to the registry when the connection is back<br>NOTIF-098 The update is saved here — it goes to the registry when the connection is back<br>NOTIF-099 The task is saved here — it goes to the registry when the connection is back |
...
| --- | --- | --- | --- | --- | --- | --- |
| MON-034 | Audit event | Audit event audit:document-flagged | The governance table binds action 'flag-document' to the audit vocabulary 'audit:document-flagged'. | — | — | Confirmed |
| MON-035 | Audit event | Audit event audit:task-updated | The governance table binds action 'update-task' to the audit vocabulary 'audit:task-updated'. | — | — | Confirmed |
| MON-036 | Audit event | Audit event audit:email-task-created | The governance table binds action 'create-task-from-email' to the audit vocabulary 'audit:email-task-created'. | — | — | Confirmed |

### Audit events written by this process

| Step | Audit event |
| --- | --- |
| STEP-0038 Mark the document | audit:document-flagged |
| STEP-0039 Update the task | audit:task-updated |
| STEP-0040 Raise a task from that email | audit:email-task-created |
...
| --- | --- | --- | --- | --- |
| VAR-017 | Support request — Wrong Task or Reference | Conditional variant selected by the reported category | A support request of category 'Wrong Task or Reference' is routed to the lookup workspace at severity high, rather than into a single support queue. | The requester selects category 'wrong-task'. |
