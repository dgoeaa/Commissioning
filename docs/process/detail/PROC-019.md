...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-021` modules/dispatch.js<br>`SRC-009` config/module-boundaries.config.js |
...
| Notifications issued | NOTIF-068 Enter who this is going to before sending<br>NOTIF-069 Dispatch recorded on this device — it has not reached the registry yet. Use Send now to try again.<br>NOTIF-070 A reason is required to mark no-dispatch<br>NOTIF-071 Recorded — nothing needs to be sent for this task<br>NOTIF-072 The registry has confirmed this dispatch<br>NOTIF-073 The registry is still unreachable — this dispatch is still waiting to be sent<br>NOTIF-074 Record closed — its dispatch lifecycle is complete |
| Downstream handoffs | — |
...
| SUBPROC-026 | Send the dispatch | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-054 | Record that nothing will be dispatched | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-055 | Send the dispatch again | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-056 | Close the dispatch | Reusable governed write | An operator activates the control bound to this action. |
