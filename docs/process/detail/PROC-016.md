...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-033` modules/reports.js<br>`SRC-009` config/module-boundaries.config.js |
...
| Notifications issued | NOTIF-127 There is nothing to print — no record falls inside the dates chosen<br>NOTIF-128 There is nothing to download — no record falls inside the dates chosen<br>NOTIF-129 Report sent by email<br>NOTIF-130 The report could not be sent right now — it is queued and will go out when the connection is back |
| Downstream handoffs | — |
...
| --- | --- | --- | --- |
| SUBPROC-028 | Produce the report | Reusable governed write | An operator activates the control bound to this action. |
