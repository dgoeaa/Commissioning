...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-016` modules/bulk-assignment.js |
...
| Notifications issued | NOTIF-040 Activity IDs — enter at least one. These are the records the assignment below is applied to.<br>NOTIF-041 We sent a 6-digit code to your registered contact. Enter it below to continue.<br>NOTIF-042 One-time code — that code does not match. Check the 6 digits and enter them again. Nothing has been assigned.<br>NOTIF-043 Saved on this device — these assignments will be sent to the registry when the connection returns |
| Downstream handoffs | — |
...
| --- | --- | --- | --- |
| SUBPROC-019 | Assign those records | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-061 | Send the one-time code | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-062 | Check the one-time code | Reusable governed write | An operator activates the control bound to this action. |
