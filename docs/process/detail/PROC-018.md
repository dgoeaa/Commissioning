...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-024` modules/executive.js<br>`SRC-009` config/module-boundaries.config.js |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CTRL-033 | Confirmation control | Operator confirmation — Append quick minute | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
| CTRL-034 | Confirmation control | Operator confirmation — Route to Assignment | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
| CTRL-035 | Confirmation control | Operator confirmation — Refresh correspondence | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
...
| Notifications issued | NOTIF-080 Select a correspondence item first<br>NOTIF-081 Classification requested — the suggestion appears on this record when it comes back<br>NOTIF-082 The classification service could not be reached. Nothing on this record has changed.<br>NOTIF-083 Enter a quick minute first<br>NOTIF-084 Minute saved locally; sync queued<br>NOTIF-085 Minute appended<br>NOTIF-086 Select an assignee<br>NOTIF-087 Opened Assignment workflow<br>NOTIF-088 Refreshing from the registry<br>NOTIF-089 The registry is unavailable — showing the records already loaded |
...
| --- | --- | --- | --- |
| SUBPROC-025 | Record the executive approval | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-041 | Return the item to the sender | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-042 | Delegate the item | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-043 | Add the minute | Reusable governed write | An operator activates the control bound to this action. |
