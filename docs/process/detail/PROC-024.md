...
| Sources | `SRC-042` config/workflow-clarity.config.js<br>`SRC-036` modules/settings.js<br>`SRC-009` config/module-boundaries.config.js |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CTRL-051 | Confirmation control | Operator confirmation — Confirm settings update | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
| CTRL-052 | Confirmation control | Operator confirmation — Use the installed addresses | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
| CTRL-053 | Confirmation control | Operator confirmation — Empty every connection address | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
| CTRL-054 | Confirmation control | Operator confirmation — Clear this device’s saved data | The operator is shown what is about to happen and must confirm before the write is attempted. | The operator activates the control this dialog guards. | Confirmation is given. | The governed write proceeds. | Declining returns to the workspace with nothing written. | Not evidenced beyond the decline path. | Confirmed |
...
| Notifications issued | NOTIF-136 Settings saved<br>NOTIF-137 Connection report copied. Signatures are removed, so it is safe to paste to IT support.<br>NOTIF-138 Could not copy. Open System Health (IT only) to read the same report on screen.<br>NOTIF-139 Boxes emptied. Press Review and save to apply it.<br>NOTIF-140 Every connection address is now empty. Nothing has changed yet — press Review and save to apply it.<br>NOTIF-141 This device’s saved data was cleared. Reloading… |
| Downstream handoffs | — |
...
| SUBPROC-006 | Endpoint Console | Sub-view of a primary workspace | — |
| SUBPROC-034 | Save your profile | Reusable governed write | An operator activates the control bound to this action. |
| SUBPROC-067 | Import that file | Reusable governed write | An operator activates the control bound to this action. |
