...
**Amended — the Entra app-roles plan below was also superseded.** This paragraph proposed
mapping the six roles onto Entra app-role claims in a bearer token. That identity provider was
removed along with the proxy (see the amendment at the top of this document): `config/auth.config.js`
now carries no `roleClaimMap` and no `provider` other than `'otp'`. The paragraph is kept for
the historical record of what was considered; treat none of it as current.

What remains is not a reconciliation but a mapping, and it is **owner-side**: under the OTP
design, `roleSource: 'verified'` reads the role `DGO_UserDirectory` (or the flow's response)
resolves for the verified identity, denying any principal it cannot resolve. The client half of
that is done and asserted — `tests/auth-posture.test.mjs` proves an unmapped claim is denied
rather than defaulted to a local role. There is no proxy start-up gate any more, so an unmapped
principal has to fail closed inside every flow, and a client-side denial is a usability
...
The internal operator arrow is also stale on its own terms: it labels the credential "Entra ID token," but that identity provider was removed with the proxy (§3.7's amendment). The internal platform carries an OTP proof — issued by `OTP_GENERATE`/`OTP_VERIFY`, the same two Power Automate flows `document-portal` would use for any identity-gated action — not an Entra token. Neither client has ever depended on Entra, Azure, or Cloudflare in what was actually built.
