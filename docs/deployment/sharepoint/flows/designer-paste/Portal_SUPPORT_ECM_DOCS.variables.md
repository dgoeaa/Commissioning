# Portal_SUPPORT_ECM_DOCS — top-level variables (create these first)
...
`Portal_SUPPORT_ECM_DOCS.designer-paste.json` — pasting them there produces a definition the
...
node scripts/patch-flow-package.mjs --in <exported.zip> --out <ready.zip> \
    --install Portal_SUPPORT_ECM_DOCS
...
`Portal_SUPPORT_ECM_DOCS.variables.designer-paste.json`, which is why that file ships. You do not
