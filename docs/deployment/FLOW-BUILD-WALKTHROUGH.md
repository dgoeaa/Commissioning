...
> **This document carries no commands.** It is the build record for the governance flows — how each was constructed. The steps for portal and endpoint
...
These have no consumer, or carried two signatures each, where regenerating the trigger could not
have revoked both. Deleting the flow is the only action that invalidates every signature it has
ever had.

> **The signatures are no longer live.** `ITEM-22` rotated the estate and reconciled it against
> [`endpoint-register.json`](../reference/endpoint-register.json): none of the disclosed set
> authenticates an endpoint this platform calls. The deletions below still stand — a flow with no
> consumer is still clutter, and a superseded signature in git history is still readable — but do
> them as housekeeping, not as an exposure response.
...
- the email-verification flows (C10) generate a one-time code, rate-limit it per address and
  per source, and compare it as a single-use value. The comparison is not constant-time — WDL has no bitwise operators, so `equals()` is what compares them; the protection is the lifecycle (short expiry, single use, attempt cap), per SC-003 / INT-005.
...
Read back the code you stored in C10.5 and compare it as a **single-use** value — the comparison
