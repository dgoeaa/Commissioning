...
- **[`DOCUMENT_PORTAL_LIVE_OPERATIONS.md`](./DOCUMENT_PORTAL_LIVE_OPERATIONS.md)** — what
  live operation actually looks like for `document-portal/` on the current baseline: which of
  its six endpoints are wired, what each does and doesn't enforce client-side, and what
  happens at every level of partial wiring. A living commentary, re-verified against commands
  rather than a frozen audit.

The scripts that replace a tedious part:

| Script | Replaces | Time saved |
|---|---|---|
| `scripts/setup-sharepoint.ps1` | Part B — 33 columns clicked one at a time | ~1 hour |
| `scripts/provision-sharepoint-fields.ps1` | The 85 columns missing across the 12 portal lists and `DGO_AccessScopes`, clicked one at a time | ~3 hours |
| `scripts/provision-sharepoint-fields.browser.js` | The same 85 columns, with nothing to install — paste into a SharePoint devtools console | ~3 hours |
| `scripts/export-power-automate-flows.ps1` | Exporting the flow definitions the coverage gate wants, one flow at a time through the portal — no app registration, no admin role | ~1 hour |
| ~~`scripts/setup-sharepoint-portal.ps1`~~ | **Superseded.** Targets 7 `NITDA_Portal_*` list names that exist in no NITDA site — see [`sharepoint/README.md`](./sharepoint/README.md) | — |

Column provisioning is documented end to end in
**[`sharepoint/README.md`](./sharepoint/README.md)** — the runbook, why the earlier attempts
could not have worked, and how to produce the sign-off evidence for the provisioning gate.
