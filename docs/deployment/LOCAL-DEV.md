...
to paste into a config file. It needs Node 22 or newer and no other software — the server
...
**It is not a production backend, and it does not replace one.**

There is no intermediary in this platform: the browser calls each Power Automate flow
directly. Validating a bearer token, deriving a role from its claims, authorizing the action
and refusing client-asserted identity are therefore each flow's own obligations, and
[`FLOW-BUILD-PLAN.md`](./FLOW-BUILD-PLAN.md) states them per contract key. That enforcement
is what makes the platform's governance real rather than advisory.

The dev server does none of it. It answers whatever it is asked, from anybody who asks. On
...
- the request body is capped, so an oversize upload is refused
  - **but the declared size and SHA-256 are *not* checked against the bytes that arrive.**
    Corrected 2026-08-31: this line claimed they were. `scripts/dev-server.mjs` caps the body
    at `maxBytes` and lets `X-DGO-Size` and `X-DGO-Sha256` through CORS, and never reads
    either. It computes no digest at all. The flow does not verify the digest either — that
    is open item 18 — so the dev server is faithful to the estate here; the sentence was not.
...
Those need a tenant, and in this topology they belong to the flows.
...
If you want the config as a real file — to use with a plain static host, say — fetch it from
the running server. It is the same text the browser gets:

```bash
npm run dev &
curl -s http://localhost:8080/config/config.local.js  > config/config.local.js
curl -s http://localhost:8080/portal/config.local.js  > document-portal/config.local.js
```

Both configs are origin-relative, so the files work under any static host on any port.

Two things to know first. The server **checks disk before generating**: if a real
`config.local.js` is already there it serves that file, so running the commands above in a
tree you have already wired copies the file onto itself — and means you cannot use them to
read the dev config while a real one exists. And once the files are on disk they are what
the dev server serves too, so the in-memory config stops applying until you delete them.

There is no `npm run setup:dev`. Earlier revisions of this page named one; it has never
existed in `package.json`. `npm run setup` is the real command, and it writes Power Automate
URLs, not dev routes.
...
**To go to a pilot:** follow [`MINIMAL-PILOT.md`](./MINIMAL-PILOT.md). Because the browser
calls the flows directly, every URL in `config.local.js` is delivered to whoever is served
the page — treat each one as a public bearer credential, build the flow to authenticate its
own callers, and rotate on a schedule.
