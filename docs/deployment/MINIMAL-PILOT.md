...
Workflow `4a250f97181b4a28abc1d0fb0f7d4c4d` carried **two** signatures, so regenerating would have
revoked only the newer one — which is why this step rebuilds rather than regenerates.

> **Not an exposure step any more.** `ITEM-22` rotated the estate; neither signature is live.
> Rebuild here if you want the flow's history clean, and skip it if you do not.
...
### 3d · Import the two intake flows — 5 minutes

These do not exist yet, but they no longer need to be built by hand: they ship pre-built in
[`docs/deployment/power-automate-flows/`](./power-automate-flows/README.md) as flows **1**
(`01-portal-submission`) and **2** (`02-portal-upload`) — complete, importable definitions,
not a description of one.

1. Create the flow with an HTTP trigger (any method), select the trigger on the canvas, and
   paste the whole contents of `power-automate-flows/paste/01-portal-submission.paste.json`
   → its URL is `DGO_ENDPOINT_INTAKE_SUBMISSION`.
2. Repeat with `02-portal-upload.paste.json` → its URL is `DGO_ENDPOINT_INTAKE_UPLOAD`.
3. Re-pick the `SharePoint` connection on each (Power Automate always drops connection
   authorisation on paste — this is normal, not a defect in the import).

If canvas paste isn't available in your designer, `power-automate-flows/README.md` §"Alternative
import paths" has the same two flows as `codeview/*.definition.json` (paste into a flow's Code
view) or as reference `*.flow.json` packages. `FLOW-BUILD-WALKTHROUGH.md`'s C7/C9 steps are kept
for reference — reading them tells you what each flow does, action by action — but building from
them by hand is no longer the fastest correct path now that the flows themselves exist.

With no proxy in front of them these two flows carry the whole security burden of the public
channel: the submission flow mints the `NITDA-YYYY-<sequence>` reference itself (unpadded,
monotonic, never restarting within a year), rate-limits by source and issues one single-use
upload ticket per attachment; the upload flow redeems that ticket once and verifies the received
bytes against the declared size and SHA-256. That is already built into both — verify it holds
by running the checks in `power-automate-flows/README.md` before trusting the import, not by
re-deriving it.

The other four flows in the same folder (`03`–`06`: support, verify-request, verify-confirm,
status) cover the portal features the minimal pilot skips for now — import them the same way,
later, if you add tracking, email verification, or the help desk (see "What you skipped" below).
...
| To add | Import from `power-automate-flows/` | Then set the key |
|---|---|---|
| Citizens tracking their submission | `06-portal-status` | `STATUS` in `PF_CONFIG` |
| Outward correspondence email | new flow — not in this set | `EMAIL` in `DGO_CONFIG` |
| Email verification before a reference is issued | `04-portal-verify-request` and `05-portal-verify-confirm` | `VERIFY` and `VERIFY_CONFIRM` in `PF_CONFIG`, then have the `SUBMISSION` flow require the proof |
| The public help desk | `03-portal-support` | `SUPPORT` in `PF_CONFIG` |
| Registry counter scan deposits | reuse the upload flow | `SCAN_INTAKE` in `DGO_CONFIG` |
| AI analysis and one-time passcodes | new flows — not in this set | the matching key in `DGO_CONFIG` |

Same import method as §3d for the four rows that ship pre-built. The two that don't
(outward email, AI/OTP) are new flows to build from scratch, the same way `3a`–`3c` regenerated
the internal ones — not by restoring anything you deleted in 3a, since those all had published
