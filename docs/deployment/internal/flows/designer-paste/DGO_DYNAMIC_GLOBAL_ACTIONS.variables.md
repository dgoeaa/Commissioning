# DGO_DYNAMIC_GLOBAL_ACTIONS — top-level variables (create these first)
...
`DGO_DYNAMIC_GLOBAL_ACTIONS.designer-paste.json` — pasting them there produces a definition the
...
node scripts/patch-flow-package.mjs --in <exported.zip> --out <ready.zip> \
    --install DGO_DYNAMIC_GLOBAL_ACTIONS
...
`DGO_DYNAMIC_GLOBAL_ACTIONS.variables.designer-paste.json`, which is why that file ships. You do not
