# DGO_SCAN_INTAKE — top-level variables (create these first)
...
`DGO_SCAN_INTAKE.designer-paste.json` — pasting them there produces a definition the
...
`DGO_SCAN_INTAKE.variables.designer-paste.json`, which is why that file ships. You do not
