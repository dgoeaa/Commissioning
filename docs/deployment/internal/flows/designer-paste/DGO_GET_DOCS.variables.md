# DGO_GET_DOCS — top-level variables (create these first)
...
`DGO_GET_DOCS.designer-paste.json` — pasting them there produces a definition the
...
`DGO_GET_DOCS.variables.designer-paste.json`, which is why that file ships. You do not
