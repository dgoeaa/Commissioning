# DGO_FETCH_ALL — top-level variables (create these first)
...
`DGO_FETCH_ALL.designer-paste.json` — pasting them there produces a definition the
...
`DGO_FETCH_ALL.variables.designer-paste.json`, which is why that file ships. You do not
