# DGO_OTP — top-level variables (create these first)
...
`DGO_OTP.designer-paste.json` — pasting them there produces a definition the
...
`DGO_OTP.variables.designer-paste.json`, which is why that file ships. You do not
