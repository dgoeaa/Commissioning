# DGO_BULK_ASSIGNMENT — top-level variables (create these first)
...
`DGO_BULK_ASSIGNMENT.designer-paste.json` — pasting them there produces a definition the
...
| `varBulkResults` | array | `[]` | `Initialize_variable_varBulkResults` |
...
`DGO_BULK_ASSIGNMENT.variables.designer-paste.json`, which is why that file ships. You do not
