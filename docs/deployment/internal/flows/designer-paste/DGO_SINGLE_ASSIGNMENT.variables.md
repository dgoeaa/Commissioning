# DGO_SINGLE_ASSIGNMENT — top-level variables (create these first)
...
`DGO_SINGLE_ASSIGNMENT.designer-paste.json` — pasting them there produces a definition the
...
`DGO_SINGLE_ASSIGNMENT.variables.designer-paste.json`, which is why that file ships. You do not
