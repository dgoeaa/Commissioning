# DGO_REFERENCE_DATA — top-level variables (create these first)
...
`DGO_REFERENCE_DATA.designer-paste.json` — pasting them there produces a definition the
...
`DGO_REFERENCE_DATA.variables.designer-paste.json`, which is why that file ships. You do not
