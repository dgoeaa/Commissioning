...
| `docs/deployment/internal/flows/designer-paste`<br>`docs/deployment/sharepoint/flows/designer-paste` | When a step says to paste a designer package. 57 files across the two, of which 34 are `*.variables.*` carriers. The carrier is pasted **before** the flow package, because the package reads declarations the carrier supplies |
