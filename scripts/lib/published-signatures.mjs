...
/**
 * The 43 characters that ARE the credential.
 *
 * A trigger signature is HMAC-SHA256 base64url: exactly 43 characters. The match above is
 * deliberately `{20,}` so a mangled short copy is still found — the corpus holds one — but it is
 * also greedy, and this corpus glues prose straight onto the end of URLs
 * (`…sig=XXXgetEmailsPOST`). So the captured STRING for one credential differs by whatever
 * followed it in the file, and that broke the two callers in opposite directions.
 *
 * Counting: the published total was inflated, because one credential indexed under several
 * strings. 55 was really 43.
 *
 * MATCHING, WHICH IS THE SERIOUS ONE: `reusedSignatures` takes a signature out of a configured
 * URL — always clean, always exactly 43 — and asks whether it is in the published index. Where
 * the corpus copy had prose attached, the index key was the longer string, the lookup missed,
 * and the gate reported the signature unpublished. That gate is what stops `npm run package`
 * building a pilot or enforced package around a credential every reader of this repository
 * already holds, and what stops `npm run commission` clearing such a deployment. It was
 * failing open on precisely the files most likely to carry prose — the operator's own labelled
 * URL lists.
 *
 * Normalising both sides is the fix: index and lookup now agree on what a signature is.
 */
export const SIG_LEN = 43;
export const normaliseSignature = (sig) => String(sig).slice(0, SIG_LEN);
...
export function publishedSignatures(root, files) {
  ...
  for (const f of files) {
    scanFile(path.join(root, f), raw => {
      const sig = normaliseSignature(raw);
...
export function reusedSignatures(values, published) {
  ...
  for (const [key, url] of Object.entries(values || {})) {
    ...
    for (const m of String(url).matchAll(SIGNATURE)) {
      const sig = normaliseSignature(m[1]);
      if (published.has(sig)) reused.push({ key, files: published.get(sig) });
    }
  }
  return reused;
}
