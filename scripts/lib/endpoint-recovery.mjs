...
const KEY_MAP = {
  ...
  /* EMPTY, AND IT HAS TO BE. See UNAVAILABLE.portal below.
   *
   * This table used to map five portal keys onto workflow ids. Four of them were ids of
   * INTERNAL flows, and none of the seven flows that actually serve the portal has a trigger
   * URL anywhere in the corpus — so there was nothing correct to put here. */
  portal: {},
};

/**
 * Keys this module cannot source a URL for, recorded so the absence is stated.
 *
 * READ THIS AS WHAT IT IS. Every entry below answers one question: can a labelled trigger URL
 * for this key be recovered from `docs/reference/`? It does NOT answer whether a flow exists.
 * The two were conflated once, and the conflation reached an external review brief as a
 * disclosure that three contract keys had no implementation — of which two, UPLOAD and
 * WRITEBACK, are deployed flows this repository holds exports of. Whether a flow exists is
 * `npm run keyimpl`'s question and it reads the definitions to answer it. This table is about
 * credentials, and a key can be absent from it and still unbuilt, or listed here and fully
 * deployed.
 */
export const UNAVAILABLE = {
  ...
  portal: {
    /* NO PORTAL ENDPOINT IS RECOVERABLE. Not one of the seven flows that serve the portal —
       CG_Submission_Endpoint, CG_Upload_Endpoint, CG_Support_Endpoint, CG_Verification_Endpoint,
       CG_Verification_Confirmation_Endpoint, CG_Status_Check_Endpoint, CG_Writeback_Endpoint —
       has a trigger URL in this corpus. The four whose workflow ids were captured from live run
       records (docs/reference/portal-endpoint-workflow-ids.json) were checked against the corpus
       index directly: no URL for any of them.

       WHAT WAS HERE BEFORE, AND WHY IT WAS WORSE THAN NOTHING. Five portal keys were mapped, and
       four of them onto INTERNAL workflow ids:

         VERIFY          -> 314aaf27… , which is the internal OTP_GENERATE flow
         VERIFY_CONFIRM  -> 43879c51… , which is the internal OTP_VERIFY flow
         STATUS          -> 85c556f1… , the internal subsidiary-actions flow, via its TRACK route
         SUPPORT         -> 85c556f1… , the same internal flow again

       The portal is invoked by anonymous strangers with no session; that is the premise the
       whole trust boundary rests on. All three of those internal flows carry
       triggerAuthenticationType "All", so they accept anonymous callers. Wiring a portal build
       this way does not merely mislabel an endpoint — it hands every visitor of the public
       portal a working URL to the internal operations estate. It was recorded as a "routed, not
       dedicated" caveat, which reads as a limitation of convenience rather than as a boundary
       crossing.

       SUBMISSION pointed at 1ff7714c…, a legacy submission flow that takes the file inline as
       base64. That one was at least a portal flow and its contract mismatch was documented, but
       it is not CG_Submission_Endpoint either.

       So the portal surface recovers nothing, and a portal package is built unwired. That is the
       correct outcome: each key reports itself unconfigured at the point of use, which is a
       portal that cannot submit, rather than a portal wired into the internal estate. */
    SUBMISSION:
      'No labelled URL in the corpus names CG_Submission_Endpoint. This key used to be wired to '
      + 'the legacy base64 submission flow 1ff7714c…, which is a different flow with a different '
      + 'contract — it answers { trackingId, referenceId, … } and takes the file inline, where '
      + 'the portal declares attachments and expects { referenceId, uploads: [ticket, …] }.',
    VERIFY:
      'No labelled URL in the corpus names CG_Verification_Endpoint. This key used to be wired '
      + 'to the INTERNAL OTP_GENERATE flow, which accepts anonymous callers — see the note above.',
    VERIFY_CONFIRM:
      'No labelled URL in the corpus names CG_Verification_Confirmation_Endpoint. This key used '
      + 'to be wired to the INTERNAL OTP_VERIFY flow, which accepts anonymous callers.',
    STATUS:
      'No labelled URL in the corpus names CG_Status_Check_Endpoint. This key used to be wired '
      + 'to the INTERNAL subsidiary-actions flow via its TRACK route; that flow accepts '
      + 'anonymous callers and serves the internal operator platform.',
    SUPPORT:
      'No labelled URL in the corpus names CG_Support_Endpoint. This key used to be wired to the '
      + 'same INTERNAL subsidiary-actions flow as STATUS, on the strength of a CREATESUPPORTREQUEST '
      + 'route being declared among its operations.',
    UPLOAD:
      'No labelled URL in the corpus names the upload endpoint. The ticket-redeeming flow '
      + 'itself EXISTS — CG_Upload_Endpoint, workflow df7ddff1, exported 2026-08-24 — and it '
      + 'performs the full redemption; what cannot be recovered is its trigger URL, so a '
      + 'package cannot be wired to it from here. The legacy submission flow, which is what the '
      + 'corpus does carry a URL for, takes bytes inline as base64 instead — the 4 MB ceiling '
      + 'the ticket design replaced — so wiring UPLOAD to it would be wrong rather than partial. '
      + 'Note also that CG_Upload_Endpoint\'s trigger is tenant-authenticated and declares POST '
      + 'while the portal PUTs, so a recovered URL alone would not make it callable.',
  },
};
