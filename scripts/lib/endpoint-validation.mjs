...
/**
 * Redact signature material from FREE TEXT — a response body, a proxy error page, a log line.
 *
 * `redact()` above takes a URL you already hold. This takes text you did not write and may not
 * have looked at, which is a different problem: the caller cannot know what is in it before
 * printing it, so the redaction has to be unconditional.
 *
 * The case that motivates it is real. `npm run verify:endpoints` prints the first 160 characters
 * of any non-JSON response so a 403 from a gateway is diagnosable rather than opaque. Gateways,
 * WAFs and reverse proxies routinely echo the request line back in an error page — "Cannot POST
 * /workflows/…?…&sig=…", nginx and Express both do it by default — and that request line is the
 * credential. The intermediary is not under our control and the body is arbitrary, so the only
 * safe assumption is that any response may quote the URL that produced it.
 *
 * Deliberately looser than `redact()`: it matches `sig=` anywhere, with or without a preceding
 * `?` or `&`, because text that has been through a proxy's own formatting may have neither. Over-
 * redacting a diagnostic costs a little context. Under-redacting one publishes a bearer token
 * into a terminal transcript that someone then pastes into an issue.
 */
export function redactText(text) {
  return String(text || '')
    .replace(/\b(sig|code)=[A-Za-z0-9_%\-+/=]+/gi, '$1=***')
