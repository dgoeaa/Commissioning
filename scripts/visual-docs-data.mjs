...
/**
 * Resolve an import specifier against the importing directory, as a repository path with
 * forward slashes on every platform.
 *
 * WHY THIS EXISTS
 * Both the module inventory and the layer graph used `path.join(...)` and then read the layer
 * off the front with `.split('/')[0]`. On Windows `join` returns `core\\api.js`, so the split
 * yields the whole string, no layer ever matches, and the graph comes out EMPTY. That is not a
 * cosmetic difference: with no edges, `acyclicIncludingDynamic` reports true — a clean bill of
 * health on the layering, issued because nothing was measured.
 */
const resolveSpec = (fromDir, spec) =>
  path.normalize(path.join(fromDir, spec)).split(path.sep).join('/');
...
function inventory(dir) {
  return listJs(dir).map(name => {
    ...
    for (const spec of [...imports, ...dynamic]) {
      const target = resolveSpec(dir, spec);
  ...
  const collect = re => {
    ...
    for (const f of files) {
      for (const m of read(f).matchAll(re)) {
        const target = resolveSpec(path.dirname(f), m[1]);
...
function portal() {
  const all = trackedFilesIn('document-portal');
...
/**
 * The files a directory holds ACCORDING TO GIT, not according to whoever's disk this is.
 *
 * Both portal counters used to read the working tree. That makes the committed dataset a
 * function of the developer's machine rather than of the repository: an operator with a
 * node_modules, an extracted package or any other untracked directory under document-portal/
 * generates a different number, `npm run test:visual` calls the committed file stale, and the
 * only way to make it pass is to commit their machine's number and break it for everyone else.
 * That happened — one tree read 41 files and another read 421 at the same commit.
 *
 * `git ls-files` is the repository's own answer and is identical in every clone. If git cannot
 * answer, this throws rather than falling back to a walk: a wrong count that looks right is
 * worse than a build that stops.
 */
function trackedFilesIn(dir) {
  const out = execFileSync('git', ['ls-files', '-z', '--', dir], { cwd: ROOT, encoding: 'utf8' });
  return out.split('\0').filter(Boolean).map((f) => f.split(path.sep).join('/')).sort();
}

function countFilesIn(dir) {
  return trackedFilesIn(dir).length;
}
