...
/* THE ESTATE BACKFILL IS RETIRED, AND IT WAS THE LAST THING STILL USING THE DEAD CORPUS.
 *
 * This used to be opt-out: a build with no --values quietly filled every unsupplied key from
 * docs/reference/foundational/, and --no-estate turned that off. The reasoning was sound while it
 * held — the exposed configuration was the only one that could be exercised live, so the packager
 * stamped the exposure rather than refusing to build.
 *
 * ITEM-22 ended that. The corpus describes the PRE-ROTATION estate: its signatures are revoked and
 * all 25 endpoint keys now point at different workflows. `npm run setup -- --recover` was retired
 * for exactly this reason, in the words the refusal still prints — "a config built from it looks
 * complete and answers 401 on every call". The packager went on doing the same thing by DEFAULT,
 * which is worse: --recover has to be asked for, and this did not. A package built without
 * --values would ship wired to dead URLs, stamped "RUNNABLE NOW".
 *
 * So the fallback is off, and the flag is kept as a no-op only so that a script or a runbook
 * still passing --no-estate does not break on an unknown argument. Supply --values. */
const NO_ESTATE = true;
if (flag('--no-estate') && !QUIET) {
  console.log('\n  note: --no-estate is now the only behaviour and the flag does nothing.');
  console.log('        The documented-estate fallback was retired with --recover: its signatures');
  console.log('        are revoked and all 25 keys point elsewhere.\n');
}
...
function buildSurface(surfaceId, tracked, published) {
  ...
  /* Nothing is REQUIRED any more, and the reason is the estate rather than a relaxation.
     Neither SCAN_INTAKE nor the portal's UPLOAD can be wired from here: no labelled trigger URL
     for either is in the corpus. The two are unwired for different reasons — SCAN_INTAKE has no
     flow at all, while UPLOAD's flow exists (CG_Upload_Endpoint) but is tenant-gated and its URL
     is not recorded — and neither reason is a gap in this build. Refusing to emit a package over
     them would leave nothing runnable at all. Each unwired key reports itself unconfigured at
     the point of use, which is the honest behaviour and is asserted by the suite. */
 ...
 *
 * ── RETIRED AS A SOURCE OF URLS, 2026-09-14 ───────────────────────────────────────────────
 *
 * Its whole content came from the documented estate, and that fallback went with --recover:
 * the corpus describes the pre-rotation estate, so every URL it held is revoked. Both halves
 * of the justification above fail now. On a default build the package wires nothing from the
 * corpus, so the catalogue would be the ONLY place carrying those dead credentials — exposure
 * the package does not otherwise have, which is the exact opposite of "adds no exposure". On a
 * --values build the package carries the operator's live URLs while the catalogue would carry
 * the corpus's dead ones: different URLs, presented as alternatives to point a key at.
 *
 * The question it existed to answer — "which other flow should this key point at?" — is now
 * answered better and without credentials by docs/reference/endpoint-register.json, exported
 * from the tenant post-rotation with signatures removed, and by tools/admin-console.html, which
 * renders all 51 workflows and says which serve a key. The file still ships, and says this,
 * because a package that silently lost a file it used to carry is worse than one that explains.
 */
function renderFlowCatalogue(surfaceId, values, meta) {
  ...
  return JSON.stringify({
    ...
    availableFlowsRetired:
      'availableFlows is empty, and that is the current correct state rather than a build '
      + 'fault. It carried the documented estate, whose trigger signatures ITEM-22 revoked: all '
      + '25 contract keys now resolve to different workflows than that corpus records. Listing '
      + 'those URLs here would ship dead bearer credentials inside a deliverable and offer them '
      + 'as flows to point a key at. For what the tenant actually holds — 25 keys across 20 '
      + 'workflows, and all 51 workflows in the environment, with every signature removed — read '
      + 'docs/reference/endpoint-register.json or open tools/admin-console.html.',
    howToRepoint:
      'To point a key at a different flow: find the flow in docs/reference/endpoint-register.json '
      + '(or tools/admin-console.html), take its trigger URL from the flow\'s own trigger card in '
      + 'Power Automate — the register carries every URL complete except the signature, which it '
      + 'may never hold — write "<PREFIX><KEY>=<url>" into a values file (prefix '
      + `${surface.envPrefixes[0]}), and rebuild with `
      + '`npm run package -- --values <file>`. There is no longer a fallback: a key you do not '
      + 'supply ships unprovisioned and says so.',
    provisioned,
    /* Empty since 2026-09-14 — see availableFlowsRetired. The fields are kept so a consumer that
       reads them gets 0 rather than undefined, and so the shape of the file does not change
       under anything already parsing it. */
