...
import { redactText } from './lib/endpoint-validation.mjs';
...
/**
 * Verify a config that is not this working tree's.
 *
 * `--config` and `--portal-config` point the run at any `config.local.js` — a built
 * package's, a colleague's, or a scratch one written by a test. Without them the only
 * configuration this script can exercise is the one in the checkout it lives in, which
 * means its contract pass cannot be tested without writing real endpoint files into the
 * repository during a test run. Both default to the working tree, so every existing
 * invocation is unchanged.
 */
const RUNTIME_CONFIG = opt('--config');
const PORTAL_CONFIG = opt('--portal-config');
...
function loadLocalConfig(relPath, globalName, override) {
  const abs = override ? path.resolve(override) : path.join(ROOT, relPath);
  if (!fs.existsSync(abs)) return {};
  const sandbox = { window: {} };
  try {
    new Function('window', fs.readFileSync(abs, 'utf8')).call(sandbox, sandbox.window);
  } catch (e) {
    console.error(`\n  ✖  ${override || relPath} does not evaluate: ${e.message}\n`);
    process.exit(2);
  }
  return sandbox.window[globalName]?.endpoints || {};
}

const runtimeEndpoints = loadLocalConfig('config/config.local.js', 'DGO_CONFIG', RUNTIME_CONFIG);
const portalEndpoints = loadLocalConfig('document-portal/config.local.js', 'PF_CONFIG', PORTAL_CONFIG);
  ...
  try {
    ...
    try {
      ...
      if (spec.expect && res.status < 400) {
        ...
        if (spec.flat) {
          /* A PORTAL endpoint must answer FLAT. document-portal/js/core.js readJson() unwraps
             nothing, and portal-data-contract.json names every field it reads at the top level.
             Finding the fields only inside `data` is not a near miss — it is the exact defect
             that made every portal endpoint answer 200 with the field out of the client's
             reach, so it is reported as a failure and named for what it is. */
          result.missing = atTop;
          result.matchedIn = 'top-level';
          if (atTop.length && inData.length < atTop.length) {
            result.wrongLevel = `answered the envelope — ${spec.expect.filter(k => !inData.includes(k)).join(', ')} is inside 'data', where the portal cannot read it`;
          }
        } else {
      ...
      result.parsed = false;
      /* Redacted before it is stored, not before it is printed. An intermediary's error page
         routinely echoes the request line — which is the signed URL — and this snippet is
         printed to a terminal people paste into issues. Redacting at the point of capture
         means no later consumer of `result` can leak what this one avoided. */
      result.snippet = redactText(text.slice(0, 160).replace(/\s+/g, ' '));
    }
  } catch (e) {
    result.ms = Date.now() - started;
...
for (const t of targets) {
  ...
  if (r.keys?.length) {
    ...
    if (r.wrongLevel) console.log(`  ${' '.repeat(24)}    ✗ ${r.wrongLevel}`);
  }
