...
PF.page = function () {
  ...
  /* Mirrors track.js's requireVerification — same round-trip, same client-half-only posture
     (README.md, D4): the flow decides whether a proof is required, the client is only ever
     capable of running the exchange when asked. A submission the flow refuses without proof
     of the address is not a failure to queue — the outbox has no way to run an interactive
     code exchange, so retrying it there would only repeat the 403 forever against a control
     that is working as intended. The submitter is standing here, so it runs once, now. */
  function requireVerification(email) {
    var panel = PF.$('#verifyPanel');
    if (!panel) return Promise.resolve(null);
    var note = PF.$('#deliveryState');
    if (note) note.textContent = 'Confirming ' + email + ' so the registry can accept this submission…';

    if (!PF.intake.verificationAvailable()) {
      panel.style.display = '';
      panel.innerHTML = '<div class="dgo-alert dgo-alert--warning"><span class="dgo-alert__icon"><svg class="icon-sm" aria-hidden="true"><use href="#i-warning"></use></svg></span>' +
        '<div class="dgo-alert__body"><div class="dgo-alert__title">Email verification is required</div>' +
        '<p style="margin:0">The registry needs proof that you read ' + PF.esc(email) + ', and this site is not configured to send that code. Your submission is saved under the tracking ID above — contact the helpdesk to have it entered.</p></div></div>';
    ...
    return new Promise(function (resolve) {
      ...
      panel.innerHTML = '<div class="pf-panel"><div class="pf-panel__body dgo-stack dgo-stack--3">' +
        ...
        '<p style="margin:0">Before the registry can accept this submission, it needs to know you read <strong>' + PF.esc(email) + '</strong>. We will send a one-time code to that address.</p>' +
        '<div id="submitVerifyStep"><button class="dgo-btn dgo-btn--primary" id="submitSendCode" type="button">Send the code</button></div>' +
        '</div></div>';

      PF.$('#submitSendCode').addEventListener('click', function () {
        var b = PF.$('#submitSendCode');
        ...
        PF.intake.verifyRequest(email).then(function (r) {
          if (!r.ok || !r.sent) {
            ...
            // the submitter to check their inbox would be a lie — the same rule track.js applies.
          ...
          PF.$('#submitVerifyStep').innerHTML =
            '<label class="dgo-field"><span class="dgo-field__label">Six-digit code</span>' +
            '<input class="dgo-input" id="submitVerifyCode" inputmode="numeric" autocomplete="one-time-code" maxlength="6"></label>' +
            '<button class="dgo-btn dgo-btn--primary" id="submitConfirmCode" type="button" style="margin-top:8px">Confirm and submit</button>';
          PF.$('#submitConfirmCode').addEventListener('click', function () {
            var code = PF.$('#submitVerifyCode').value.trim();
            if (!/^\d{6}$/.test(code)) { PF.toast('error', 'Check the code', 'Enter the six digits from the email.'); return; }
            var c = PF.$('#submitConfirmCode');
            ...
            PF.intake.verifyConfirm(email, code).then(function (v) {
              ...
              panel.innerHTML = '';
              resolve(v.verification);   // single use — consumed by the retry below, then discarded
            });
          });
          PF.$('#submitVerifyCode').focus();
        });
      });
    });
  }
  ...
  function dispatchToWorkflow(rec) {
    ...
    /* Declare a digest per attachment. It is RECORDED, NOT ENFORCED — this comment used to say
       "so the upload flow can verify that what arrives is what was described", and the flow
       cannot: Workflow Definition Language has no hash function, and the operator confirmed on
       2026-09-03 that this tenant cannot run the inline-code action that would be the only way.
       See register ITEM-18. The value is stored as DeclaredSha256 and supports an out-of-band
       comparison later; nothing checks it at intake, and nothing here may say otherwise. The
       size check IS enforced and answers 422 on a mismatch.

       Files restored from a draft have no bytes and are declared without one — they are
       reported as undelivered below rather than silently skipped. */
    ...
    return Promise.all(withBytes.map(function (f) { return digestOf(f.file); }))
      .then(function (digests) {
        ...
        function attempt(verification) {
          return PF.intake.submit(payload, { verification: verification }).then(function (res) {
            if (!res.delivered) {
              if (res.reason === 'verification-required' && !verification) {
                return requireVerification(rec.email).then(function (proof) {
                  return proof ? attempt(proof) : 'queued';
                });
              }
              return 'queued';
            }
  ...
  function uploadAll(tickets, withBytes, ref) {
    ...
    function next() {
      ...
      return PF.intake.upload(t.ticket, f.file).then(function (r) {
        // `stored:false` is the upload flow saying it accepted the PUT but did not keep the
        // bytes — a digest mismatch or an oversize file, per PF.intake.upload's contract.
        // Reading only `r.ok` reported that as delivered.
        if (!r.ok || !r.stored) failed.push(f.name);
        next();
      });
    }
    next();
    ...
    PF.$('#result').innerHTML =
      ...
      '<div class="pf-result">' +
        ...
        '<div style="padding:22px;display:grid;gap:20px;background:var(--dgo-color-surface-raised)">' +
          ...
          '</div>' +
          '<div id="verifyPanel" class="pf-no-print" style="display:none"></div>' +
