...
PF.page = function () {
  ...
  function send(data) {
    ...
    /* The reference shown to the citizen must be one the helpdesk can find. sref() mints a
       provisional reference so a case exists on this device the instant it is filed — offline
       or delivered, the citizen always has something to quote — but it is a local id, not the
       workflow's, and the two were never reconciled. Awaiting the response and adopting its
       caseRef is what makes the printed receipt and the one the helpdesk sees the same case. */
    PF.intake.support({
      name: ticket.name,
      email: ticket.email,
      ...
      if (res.delivered && res.caseRef && res.caseRef !== ticket.ref) {
        PF.store.updateTicket(ticket.ref, { ref: res.caseRef });
        ticket.ref = res.caseRef;
      }
    }).then(function () {
