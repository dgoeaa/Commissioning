...
(function () {
  ...
  PF.intake = {
    ...
    /* D13 / INT-007 · citizen write-back. A verified submitter responds, adds a note, or
       withdraws. There is no account and no session — the proof from verifyConfirm is the
       only authentication PORTAL_DATA_CONTRACT.md's WRITEBACK contract accepts, read, matched
       and consumed on every call. So unlike submit and support, a failed call is never queued
       in the outbox: a queued retry would carry a proof already spent or expired, which is a
       worse failure than telling the citizen now that it did not go through. */
    writeback: function (referenceId, verification, action, body) {
      var url = endpointUrl('WRITEBACK');
      if (!url) return Promise.resolve({ ok: false, reason: 'not-configured' });
      if (!navigator.onLine) return Promise.resolve({ ok: false, reason: 'offline' });
      return fetch(url, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ referenceId: referenceId, verification: verification, action: action, body: body })
      }).then(function (r) {
        return readJson(r).then(function (data) {
          if (!r.ok) return { ok: false, status: r.status, reason: data.reason || data.error || 'refused' };
          return { ok: data.ok === true, queued: data.queued === true };
        });
      }).catch(function () { return { ok: false, reason: 'unreachable' }; });
    },
    ...
    /* Write-back needs all three: somewhere to send the writeback itself, and both halves of
       the verify round-trip to mint the proof it requires on every call. */
    writebackAvailable: function () {
      return !!endpointUrl('WRITEBACK') && PF.intake.verificationAvailable();
    }
  };
