...
export const DEFAULT_ROUTING_CATEGORY = 'Unclassifieds';

/**
 * Document kind → approved category.
 *
 * APPROVED by the agency on 2026-09-08, closing MANUAL-1. This is no longer provisional and no
 * longer a reading of the fallback matrix: the operating model is the approved category matrix
 * in config/category-matrix.config.js, and this table is the bridge from the eleven document
 * kinds a submitter picks to the categories that matrix routes on.
 *
 * Anything absent falls to DEFAULT_ROUTING_CATEGORY. Under the approved matrix that is
 * "Unclassifieds", which the matrix carries as a row of its own — an unclassified item is
 * registered as unclassified rather than silently pushed onto a desk that did not ask for it.
 */
export const DocumentKindRouting = Object.freeze({
  'Ministerial Directive':   'FMCIDE',
  'Policy Submission':       'NITDA Services',
  'Compliance Filing':       'NITDA Services',
  'Application':             'NITDA Services',
  'Project Proposal':        'Contracts and Services',
  'Proposal':                'Sponsorship And Collaborations',
  'Report':                  'NITDA Internal Operations',
  'Meeting Request':         'Engagements and Events',
  'Event Invitation':        'Engagements and Events',
  'Official Correspondence': 'Administrative Matters',
  'General Correspondence':  'Administrative Matters',
});
