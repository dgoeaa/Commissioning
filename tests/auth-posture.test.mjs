...
if (POSTURE === 'enforced') {
  ...
  /* SCAN_INTAKE TAKES ITS ACTOR FROM THE SERVER, AND THIS IS WHY THAT IS CHECKED HERE.
     Every other internal flow resolves the caller from `userEmail` in the request body — the
     fail-open this estate documents — and SCAN_INTAKE cannot, because its body is the raw bytes
     of a scanned file. modules/scan-intake.js writes `res.depositedBy` into the custody record,
     so a body-derived actor would be an unauthenticated string in a custody trail.

     It was built wrong and shipped wrong. The builder's `gate()` wrapper took four parameters
     and the caller override had been added only to `identityGate()`, so the principal-header
     expression was passed into a fifth parameter that did not exist and was dropped without a
     word. The generated package read the body like all the others. It failed CLOSED — a binary
     body yields no userEmail, the directory lookup matches nothing, every caller gets 401 — so
     nothing was exposed, but the flow was inert and the claim that it takes identity from the
     server was false as built.

     Nothing caught it: the schema, forensic and response-contract checks all verify SHAPE, and
     the shape was correct. This checks the one expression that carries the meaning. */
  const scanPkg = JSON.parse(readFileSync(new URL(
    '../docs/deployment/internal/flows/designer-paste/DGO_SCAN_INTAKE.designer-paste.json', import.meta.url), 'utf8'));
  const scanFlat = JSON.stringify(scanPkg);
  const callerExpr = (() => {
    const seen = [];
    (function walk(a) {
      for (const [n, v] of Object.entries(a || {})) {
        if (!v || typeof v !== 'object') continue;
        if (n === 'Compose_Scan_Caller') seen.push(v.inputs);
        walk(v.actions); walk(v.else?.actions);
      }
    })(scanPkg.serializedValue?.actions);
    return seen;
  })();
  check('SCAN_INTAKE declares exactly one caller expression',
    callerExpr.length === 1, `found ${callerExpr.length}`);
  check('SCAN_INTAKE takes its actor from the tenant-authenticated principal header',
    String(callerExpr[0] || '').includes("headers']?['x-ms-client-principal-name"), String(callerExpr[0]));
  check('SCAN_INTAKE never derives its actor from the request body',
    !String(callerExpr[0] || '').includes('triggerBody'), String(callerExpr[0]));
  /* The deposit itself must still be gated on the directory, not merely on who the header says
     you are — a principal header is an identity, not an authorisation. */
  check('SCAN_INTAKE still resolves that actor against DGO_UserDirectory',
    scanFlat.includes('Get_Scan_Directory_User'));
}
