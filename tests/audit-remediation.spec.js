...
/**
 * Assert the precondition these demo-mode tests silently depend on.
 *
 * `document-portal/config.local.js` is git-ignored and loaded with `onerror="void 0"`, so a
 * copy left behind by a commissioning rehearsal changes what the portal IS without changing
 * anything a diff would show. The findings below measure the unconfigured portal: with a stale
 * config present the portal is configured, dispatch fails against an unreachable flow instead
 * of being refused for want of one, and V-02 reports `queued` where the finding is about
 * `held`. The test then fails truthfully but names the symptom — "expected 'Saved on this
 * device only', got 'Saved — not yet delivered'" — which reads as a copy regression and sent
 * one investigation down the wrong path.
 *
 * Checked explicitly so the failure names the cause and the file to delete.
 */
async function expectUnconfigured(page) {
  const configured = await page.evaluate(() => PF.backendConfigured());
  expect(configured, 'this test measures the UNCONFIGURED portal, but a SUBMISSION endpoint is '
    + 'set — delete document-portal/config.local.js (git-ignored, usually left by a '
    + 'commissioning rehearsal) and re-run').toBe(false);
}
...
test.describe('H-02 / V-06 · a real icon set, not typographic characters', () => {
  test('every navigation and top-bar icon is a resolved sprite symbol', async ({ page }) => {
    ...
    // 25 since the Admin Suite was added to SYSTEM as a sidebar destination.
    expect(r.navSvgs).toBe(25);
...
test.describe('I-09 · the intake toolbar wraps instead of pinning its primary action to the edge', () => {
  /* The finding was measured at a 700px main width: "Tracker" abutted the right boundary and
     the placeholder truncated to "Search records, IDs, sender or referen". Both are properties
     of the rendered row, so both are measured here rather than asserted from the source. */
  for (const w of [1440, 900, 700]) {
    test(`filters and actions are two rows below 900px — at ${w}px`, async ({ page }) => {
      await page.setViewportSize({ width: w, height: 900 });
      await enterPlatform(page);
      await page.evaluate(() => { location.hash = '#/correspondence'; });
      await page.waitForTimeout(900);
      const r = await page.evaluate(() => {
        const f = document.querySelector('.cc-controls__filters');
        const a = document.querySelector('.cc-controls__actions');
        const q = document.querySelector('[data-q]');
        const primary = document.querySelector('[data-new]');
        const pane = document.querySelector('.cc-controls').getBoundingClientRect();
        return {
          sameRow: Math.abs(f.getBoundingClientRect().top - a.getBoundingClientRect().top) < 4,
          // The whole complaint: the primary action sitting on the pane boundary.
          primaryGap: Math.round(pane.right - primary.getBoundingClientRect().right),
          // scrollWidth exceeds clientWidth exactly when the placeholder is being cut.
          placeholderFits: q.scrollWidth <= q.clientWidth + 1,
          tabs: [...document.querySelectorAll('.cc-tab')].map(t => t.textContent.trim()),
        };
      });
      if (w <= 900) expect(r.sameRow, 'the toolbar should be on two rows here').toBe(false);
      else expect(r.sameRow, 'the toolbar should fit one row here').toBe(true);
      expect(r.primaryGap, '"Log New Memo" is against the pane edge').toBeGreaterThan(8);
      expect(r.placeholderFits, 'the search placeholder is truncated').toBe(true);
      // Both record views name themselves; neither hides behind an unlabelled "···".
      expect(r.tabs.length).toBe(2);
      expect(r.tabs.every(t => t.length > 1)).toBe(true);
    });
  }
});
...
test.describe('I-20 / I-22 · controls carry names, prose is not styled as a link', () => {
  ...
  test('Assignment Desk source cards do not underline their body text', async ({ page }) => {
    await enterPlatform(page);
    /* The picker these cards belong to is the "nothing selected" state, and the I-13
       empty-state contract (Phase 5) split that from the two states it used to be confused
       with: a load that failed, and a registry that genuinely holds nothing. Under test the
       endpoints are unconfigured, so `runtime.lastLoad.ok` is false and the desk correctly
       says the registry could not be reached rather than inviting the operator to choose
       from four sources it never managed to read. A successful load holding one record is
       the state in which the picker IS the right screen — seeded here rather than asserted
       away, because the property under test is the card's text-decoration, not which empty
       state precedes it. */
    await page.evaluate(async () => {
      const { State } = await import('./core/state.js');
      State.patch({
        activities: [{ id: 'ACT-1', title: 'Seeded record', status: 'Pending' }],
        runtime: { ...State.get().runtime, lastLoad: { ok: true, at: new Date().toISOString() } },
      }, { module: 'test', action: 'seed' });
    });
