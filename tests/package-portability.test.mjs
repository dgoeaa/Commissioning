...
function trackedFiles() {
  return askGit({ root: ROOT, what: 'the files a clone contains' });
}
...
section('Code that runs on Windows');

/* A file URL's `.pathname` is not a filesystem path. On Windows
   `new URL('..', import.meta.url).pathname` is `/C:/repo/`, and feeding that to
   path.resolve() prepends the working directory: `C:\\C:\\repo`. Every read then fails
   with ENOENT on a path containing the drive letter twice — and it fails only on
   Windows, so a POSIX CI run stays green while the operator running the documented
   command cannot start. `fileURLToPath()` is the conversion; there is no reason to
   reach for `.pathname` when a path is wanted.

   The same applies to `import.meta.url === \`file://${process.argv[1]}\``: on Windows
   argv[1] is `C:\\repo\\script.mjs`, which never equals the `file:///C:/repo/...`
   form, so the main-module guard is silently false and the script does nothing.
   `pathToFileURL(process.argv[1]).href` is the comparison that holds on both. */

/* Matches `new URL('literal', base).pathname` and any `.pathname` on a line that
   mentions import.meta.url. It deliberately does NOT match `new URL(variable).pathname`
   or `parsed.pathname`, which are how a genuine http URL is taken apart. */
const PATHNAME_AS_PATH = /new URL\(\s*['"`][^)]*\)\.pathname|import\.meta\.url[^\n]*\.pathname/;
const HANDBUILT_FILE_URL = /file:\/\/\$\{/;

/* This file is excluded from its own scan: it has to carry both broken shapes as
   literals, in the patterns above and in the bait below. */
const SELF = 'tests/package-portability.test.mjs';
const CODE = files.filter(p => /^(scripts|tests)\/.+\.(mjs|js)$/.test(p) && p !== SELF);

t('no filesystem path is taken from a file URL\'s .pathname', () => {
  const bad = [];
  for (const rel of CODE) {
    const src = readFileSync(path.join(ROOT, rel), 'utf8');
    src.split('\n').forEach((line, i) => {
      if (PATHNAME_AS_PATH.test(line)) bad.push(`${rel}:${i + 1}  ${line.trim()}`);
    });
  }
  assert.deepEqual(bad, [],
    'use fileURLToPath(new URL(...)) — .pathname yields /C:/... on Windows');
});

t('no main-module guard compares against a hand-built file:// string', () => {
  const bad = [];
  for (const rel of CODE) {
    const src = readFileSync(path.join(ROOT, rel), 'utf8');
    src.split('\n').forEach((line, i) => {
      if (HANDBUILT_FILE_URL.test(line)) bad.push(`${rel}:${i + 1}  ${line.trim()}`);
    });
  }
  assert.deepEqual(bad, [],
    'use pathToFileURL(process.argv[1]).href — a Windows argv[1] never matches file://${...}');
});

t('the guard above would catch the shape it is written for', () => {
  /* Bait, so a rewrite of the pattern that stops matching anything fails here rather
     than passing vacuously. */
  assert.ok(PATHNAME_AS_PATH.test("const ROOT = resolve(new URL('..', import.meta.url).pathname);"),
    'the .pathname pattern no longer matches the bug it was written for');
  assert.ok(PATHNAME_AS_PATH.test("execFileSync(node, [new URL('scripts/x.mjs', root).pathname]);"),
    'the .pathname pattern no longer matches the second shape of the bug');
  assert.ok(!PATHNAME_AS_PATH.test('badResponses.push(new URL(url).pathname);'),
    'the .pathname pattern now flags taking apart a genuine http URL');
  assert.ok(HANDBUILT_FILE_URL.test('if (import.meta.url === `file://${process.argv[1]}`) {'),
    'the file:// pattern no longer matches the bug it was written for');
});
