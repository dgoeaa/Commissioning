 ...
 * bytes must be verified, and the status denial must be uniform — because a dev server that
 * is laxer than the flow contract teaches the wrong thing about how the portal behaves.
 *
...
await t('the runtime config is generated, not written into the repository', () => {
  /* Writing these would mean `git status` noise and eventually a generated file in a commit.
     They are served from memory instead.

     This used to assert the two files simply do not EXIST. That is a different claim, and a
     false one: `docs/deployment/COMMISSIONING.md` instructs every operator to run
     `npm run setup`, whose entire job is to scaffold `config/config.local.js` and
     `document-portal/config.local.js`. So an operator who followed the documented path could
     never get a green run, and the only way to pass was to delete the configuration the guide
     had just told them to create. The property worth holding is that the dev server does not
     write them — which is about what CHANGES, not about what is there. */
  const paths = ['config/config.local.js', 'document-portal/config.local.js'];
  const probe = `
    import fs from 'node:fs';
    import path from 'node:path';
    const paths = ${JSON.stringify(paths)};
    const stamp = () => paths.map((p) => {
      try {
        const s = fs.statSync(path.join(${JSON.stringify(REPO)}, ...p.split('/')));
        return \`\${s.size}:\${s.mtimeMs}\`;
      } catch { return 'absent'; }
    });
    const before = stamp();
    const { appConfigScript, portalConfigScript } =
      await import(${JSON.stringify(new URL('../scripts/dev/runtime-config.mjs', import.meta.url).href)});
    appConfigScript(); portalConfigScript();
    console.log(JSON.stringify({ before, after: stamp() }));
  `;
  /* A child process, so an import-time write is caught too — by the time this file's own
     imports have run, such a write would already have happened. */
  const out = execFileSync(process.execPath, ['--input-type=module', '-e', probe],
    { cwd: REPO, encoding: 'utf8' });
  const { before, after } = JSON.parse(out.trim().split('\n').pop());
  assert.deepEqual(after, before,
    'generating the runtime config changed a config.local.js on disk — it must be served from memory');
...
await t('SCAN_INTAKE is wired even though it is not a contract key', () => {
  // It is a raw-bytes PUT rather than a JSON POST, so it is absent from EndpointContracts and
  // therefore from EndpointKeys. Leaving it out makes core/scan-intake-service.js report
  // itself unconfigured and Registry Scan Intake untestable locally — a silent gap.
  assert.match(appConfigScript(), /"SCAN_INTAKE": "\/api\/documents\/scan"/);
});

await t('the portal config names every flow the portal resolves', () => {
  /* The portal reads PF_CONFIG.endpoints[NAME], one URL per flow — there is no base URL to
     prefix, because there is no intermediary. Miss SUBMISSION and the portal silently stays
     in demo mode; miss any other and that feature reports itself unconfigured. */
  const script = portalConfigScript();
  const missing = ['SUBMISSION', 'UPLOAD', 'SUPPORT', 'VERIFY', 'VERIFY_CONFIRM', 'STATUS']
    .filter(k => !script.includes(`"${k}"`));
  assert.deepEqual(missing, [], `not wired: ${missing.join(', ')}`);
  assert.ok(!/proxyBaseUrl/.test(script), 'the portal resolves per-flow URLs, not a base URL');
  assert.ok(!/proxyBaseUrl/.test(appConfigScript()), 'there is no proxy to point the app at');
});
