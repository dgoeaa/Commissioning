...
t('a package wired to published signatures is stamped, not refused', () => {
  ...
  /* A SURFACE THAT RECOVERS NOTHING IS NOT A WEAKER VERSION OF ONE THAT DOES.
     This asserted the wired contract on every surface, which held only while the portal keys
     were mapped onto internal flows. Once that mapping was withdrawn — no portal flow has a
     trigger URL in the corpus — the portal builds unwired, and that is the correct outcome, not
     a regression. Both contracts are asserted here rather than the wired one being relaxed to
     fit: an unwired surface must still declare itself, and must NOT claim to be runnable. */
  for (const id of SURFACE_IDS) {
    ...
    if (m.provisionedCount > 0) {
      assert.equal(m.demo, false, `${id}: a wired package must not be a demo`);
      ...
      assert.equal(m.demo, true, `${id}: a package with nothing provisioned must say it is a demo`);
      assert.ok(!/RUNNABLE NOW/.test(deploy),
        `${id}: nothing is wired, so DEPLOY.md must not claim the package is runnable`);
    }
  }
});

t('the package ships no revoked signature, and says where the live list is', () => {
  /* THIS ASSERTION WAS REVERSED ON 2026-09-14, AND THE REVERSAL IS THE POINT.
   *
   * It used to require the package to carry 39+ complete, unredacted trigger URLs from the
   * documented estate, so that an operator debugging a probe could see which other flow a key
   * might point at. That was right while those URLs worked, and the file justified itself on the
   * grounds that it "adds no exposure beyond what serving this package already creates".
   *
   * ITEM-22 revoked every one of them. Both halves of that justification then failed: a default
   * build now wires nothing from the corpus, so the catalogue would be the ONLY place carrying
   * the dead credentials; and a --values build carries the operator's live URLs while the
   * catalogue would offer the corpus's dead ones as alternatives to point a key at.
   *
   * So the contract is now the opposite, and is asserted as such rather than deleted: the
   * catalogue ships, carries no signature, and names where the live list is. The question it
   * existed to answer is answered by docs/reference/endpoint-register.json and the admin
   * console, both of which hold the tenant's 51 workflows with signatures removed. */
  /* Built at `catalogue` because three tests below reuse this build rather than repeating a
     ~10s package run. That dependency was implicit when the retired test owned this
     directory; it is named here so deleting this test does not silently break them —
     which is exactly what happened when the retired one went. */
  const out = path.join(work, 'catalogue');
  const r = pack(['--out', out, '--quiet']);
  assert.equal(r.code, 0, r.stdout);

  const SIG = new RegExp(`${SIG_PARAM}=([A-Za-z0-9_-]{20,})`);
  for (const id of SURFACE_IDS) {
    const dir = path.join(out, SURFACES[id].packageName);
    const file = path.join(dir, 'FLOW_CATALOGUE.json');
    assert.ok(fs.existsSync(file),
      `${id}: FLOW_CATALOGUE.json must still ship — a package that silently loses a file it used `
      + 'to carry is worse than one that explains why the file is now empty');
    const raw = fs.readFileSync(file, 'utf8');
    const cat = JSON.parse(raw);

    assert.equal(cat.availableFlows.length, 0,
      `${id}: the catalogue lists ${cat.availableFlows.length} flows from the retired corpus`);
    assert.equal(cat.availableFlowCount, cat.availableFlows.length,
      `${id}: the catalogue's own count disagrees with what it lists`);
    assert.ok(/revoked|ITEM-22/.test(cat.availableFlowsRetired || ''),
      `${id}: the catalogue must say WHY it is empty, or it reads as a build fault`);
    assert.match(cat.availableFlowsRetired || '', /endpoint-register\.json|admin-console/,
      `${id}: it must name where the live list is`);

    assert.ok(!SIG.test(raw),
      `${id}: FLOW_CATALOGUE.json still carries a signature — every one of them is revoked`);
  }

  /* And the retirement is real, not just absent from this one file: a default build must not
     wire a single endpoint from the corpus. */
  for (const id of SURFACE_IDS) {
    const dir = path.join(out, SURFACES[id].packageName);
    const m = JSON.parse(fs.readFileSync(path.join(dir, 'PACKAGE_MANIFEST.json'), 'utf8'));
    assert.equal(m.provisionedCount, 0,
      `${id}: a build with no --values provisioned ${m.provisionedCount} endpoint(s) — the `
      + 'documented-estate fallback was retired with --recover; its URLs answer 401');
  }
});

t('the manifest redacts, whatever else a package carries', () => {
  /* Preserved from the retired catalogue test, which asserted this as an aside. It is the half
     that survives the corpus retirement: PACKAGE_MANIFEST.json is meant to be shareable — pasted
     into an issue, attached to a ticket — so a signature reaching it is a disclosure regardless
     of where the URLs came from. */
  const out = path.join(work, 'redaction');
  const vals = writeValues(work, { runtimeKeys: pilotKeysOf('runtime'), portalKeys: pilotKeysOf('portal') });
  const r = pack(['--out', out, '--quiet', '--values', vals]);
  assert.equal(r.code, 0, r.stdout);
  const SIG = new RegExp(`${SIG_PARAM}=([A-Za-z0-9_-]{20,})`);
  for (const id of SURFACE_IDS) {
    const dir = path.join(out, SURFACES[id].packageName);
...
t('the self-contained copy needs no sibling file', () => {
  ...
  for (const id of SURFACE_IDS) {
    ...
    /* It must carry the same URLs as the package it came from, not a stale or empty set.
       The "not empty" guard is kept, but scoped to surfaces that actually provisioned something:
       the portal recovers no URLs at all now, and demanding one there would only be satisfiable
       by putting back the mapping that pointed portal keys at internal flows. */
    const cfg = fs.readFileSync(path.join(dir, SURFACES[id].configPath), 'utf8');
    const urls = [...cfg.matchAll(/"(https:\/\/[^"]+)"/g)].map(m => m[1]);
    const m = JSON.parse(fs.readFileSync(path.join(dir, 'PACKAGE_MANIFEST.json'), 'utf8'));
    if (m.provisionedCount > 0) {
      assert.ok(urls.length > 0, `${id}: provisioned ${m.provisionedCount} key(s) but the configuration carries no URL`);
    }
    for (const u of urls) {
