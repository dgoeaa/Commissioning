...
/* ------------------------------------------------------------------ *
 * The snippet is third-party text, and it is printed.
 *
 * `verify:endpoints` prints the first 160 characters of any non-JSON response, which is what
 * makes a gateway 403 diagnosable instead of opaque. But that body is written by whatever
 * answered — a proxy, a WAF, a load balancer — and echoing the request line back is what those
 * things do by default. nginx and Express both do it. The request line is the signed URL.
 *
 * So the one place this tool prints text it did not compose is also the one place a bearer
 * credential can arrive from outside and be printed to a terminal that someone then pastes into
 * an issue, a ticket or a chat message. The transcript is the leak, not the network.
 *
 * The stand-in below answers exactly as a terse gateway does. The assertion is not that the
 * output is tidy — it is that the signature is absent from it. Measured, not assumed: with the
 * redaction removed, this fails.
 * ------------------------------------------------------------------ */

const CANARY = 'C4N4RY' + 'a'.repeat(37);   // 43 characters, the length of a real signature

const echoer = await startServer(`(req, res) => {
  res.writeHead(403, { 'Content-Type': 'text/plain' });
  res.end('Denied: ' + req.url);
}`);

/* The snippet is printed by the CONTRACT pass, which reads config.local.js — so this needs a
   config of its own. `--config` exists for exactly this: without it the only configuration
   this script can be tested against is the repository's own, and a test that writes real
   endpoint files into the working tree to exercise a code path is a worse thing than the
   path it is testing. */
const scratchConfig = path.join(work, 'echo.config.local.js');
fs.writeFileSync(scratchConfig,
  'window.DGO_CONFIG = { endpoints: { FETCH_ALL: '
  + JSON.stringify(`${echoer.base}/wf?si` + `g=${CANARY}`) + ' } };\n');

const echoRun = spawnSync(process.execPath, [
  SCRIPT, '--config', scratchConfig, '--portal-config', '/nonexistent',
  '--json', reportPath, '--only', 'FETCH_ALL',
], { cwd: ROOT, encoding: 'utf8' });
echoer.stop();

check('a signature echoed back by a gateway is not printed to the transcript', () => {
  const text = `${echoRun.stdout}${echoRun.stderr}`;
  assert(/Denied:/.test(text),
    `the echoed body was never printed, so this proves nothing:\n${text.slice(-800)}`);
  assert(!text.includes(CANARY),
    'the response body was printed verbatim and carried the signature the request was made '
    + 'with — a bearer credential, in output people paste into issues');
  assert(!text.includes(CANARY), 'signature present in stdout');
  const raw = fs.readFileSync(reportPath, 'utf8');
  assert(!raw.includes(CANARY), 'the JSON report carries the echoed signature');
});
