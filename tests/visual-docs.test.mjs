...
t('the committed dataset is what the generator produces now', () => {
  ...
  const strip = o => JSON.parse(JSON.stringify(o, (k, v) => (VOLATILE.has(k) ? undefined : v)));
  const a = strip(JSON.parse(fresh));
  const b = strip(JSON.parse(committed[1]));
  if (JSON.stringify(a) === JSON.stringify(b)) return;

  /* WHY THIS REPORTS PATHS RATHER THAN DEEP-EQUALLING
     assert.deepEqual on this dataset prints one 74-megabyte line and truncates it, so the
     operator learns that something drifted and nothing about what. That is worse than useless
     on a check whose whole job is to name drift: it happened, twice, and both times the cause
     had to be found by hand. So the comparison walks the two trees and names the first paths
     that disagree, with both values abbreviated. */
  const diffs = [];
  const abbrev = (v) => {
  ...
  const walk = (x, y, at) => {
    ...
    const both = (v) => v && typeof v === 'object';
    if (both(x) && both(y) && Array.isArray(x) === Array.isArray(y)) {
      const keys = [...new Set([...Object.keys(x), ...Object.keys(y)])];
      /* An array whose LENGTH changed is reported as one fact, not as every shifted index. */
      if (Array.isArray(x) && x.length !== y.length) {
        diffs.push(`${at}.length: generated ${x.length}, committed ${y.length}`);
        return;
      }
      for (const k of keys) walk(x[k], y[k], at ? `${at}.${k}` : k);
      return;
    }
    diffs.push(`${at}: generated ${abbrev(x)} · committed ${abbrev(y)}`);
  };
  walk(a, b, '');
  assert.fail('docs/visual/platform-data.js is stale — run `npm run visual`\n     '
    + (diffs.length ? diffs.join('\n     ') : 'the two differ but no path could be isolated')
    + (diffs.length >= 12 ? '\n     (first 12 shown)' : ''));
});
...
/* The portal counts must come from the REPOSITORY, not from whoever's disk generated them.
   Both portal counters used to walk the working tree, so an untracked node_modules, an
   extracted package, or any other stray directory under document-portal/ changed the number:
   one clone read 41 files and another read 421 at the same commit, and the only way to make
   `npm run visual` agree was to commit one machine's answer and break it for everyone else.
   `git ls-files` is identical in every clone, which is the whole point. */
/* An EMPTY layer graph is the dangerous outcome, not a stale one. Both graph builders resolved
   an import with `path.join` and then read the layer off the front with `.split('/')[0]`; on
   Windows join returns `core\\api.js`, so no layer ever matched and the graph came out with
   zero edges — at which point `acyclicIncludingDynamic` reports TRUE, a clean bill of health on
   the layering issued because nothing was measured. A vacuous pass is worse than a failure. */
t('the layer graph was actually measured, not silently empty', () => {
  const LAYERS = ['config', 'core', 'shared', 'modules'];
  const keys = Object.keys(P.layers.edges);
  assert.ok(keys.length > 0, 'layers.edges is empty — the import graph resolved nothing');
  for (const k of keys) {
    const [a, b] = k.split('->');
    assert.ok(LAYERS.includes(a) && LAYERS.includes(b),
      `edge "${k}" does not name two layers — a path separator leaked into the key`);
    assert.ok(Number.isInteger(P.layers.edges[k]) && P.layers.edges[k] > 0,
      `edge "${k}" carries no count`);
  }
  assert.ok(Object.keys(P.layers.dynamicEdges).length > 0,
    'layers.dynamicEdges is empty — the composition root reaches its modules dynamically');
  /* And the per-module view must agree that something reaches somewhere. */
  const anyReach = LAYERS.some((l) => (P.inventory[l] || []).some((m) => m.reaches.length > 0));
  assert.ok(anyReach, 'no module in any layer reaches another — the resolver returned nothing');
});

t('the generator normalises a resolved import to forward slashes', () => {
  const src = readFileSync(path.join(ROOT, 'scripts/visual-docs-data.mjs'), 'utf8');
  /* One helper resolves a specifier, and it is the ONLY place path.join's output is produced
     for that purpose. Asserting "a normalisation appears somewhere in the file" is not enough —
     this file has another for a different job, and it satisfied a looser rule while the graph
     resolver was left un-normalised. */
  assert.match(src, /const resolveSpec = [\s\S]{0,200}?\.split\(path\.sep\)\.join\('\/'\)/,
    'resolveSpec must normalise the separator before any layer is read off the front');
  assert.equal((src.match(/path\.normalize\(path\.join\(/g) || []).length, 1,
    'a resolved path is being built outside resolveSpec — it will carry backslashes on Windows');
  assert.ok((src.match(/resolveSpec\(/g) || []).length >= 2,
    'both the inventory and the layer graph must resolve through resolveSpec');
});

t('the portal file count is what git tracks, not what happens to be on disk', () => {
  const tracked = execFileSync('git', ['ls-files', '-z', '--', 'document-portal'],
    { cwd: ROOT, encoding: 'utf8' }).split('\0').filter(Boolean);
  assert.equal(P.portal.files, tracked.length,
    'portal.files disagrees with git ls-files — the generator is reading the working tree again');
  const zone = P.zones.find((z) => z.id === 'public').components[0].detail;
  assert.ok(zone.startsWith(`${tracked.length} files`),
    `the public zone says "${zone}" but git tracks ${tracked.length} file(s)`);
});

t('the generator asks git for the portal inventory rather than walking it', () => {
  const src = readFileSync(path.join(ROOT, 'scripts/visual-docs-data.mjs'), 'utf8');
  assert.match(src, /execFileSync\('git', \['ls-files'/,
    'the portal inventory must come from git ls-files');
  assert.ok(!/function countFilesIn\([\s\S]{0,400}readdirSync/.test(src),
    'countFilesIn is walking the filesystem again');
});
...
t('regenerating is a documented one-liner', () => {
  ...
  /* Reachable from `npm test`, not literally inside it. `test` used to name all sixty-one
     suites in one string; it is now `test:node && test:smoke`, and a match on the top level
     would have said the atlas check had been dropped when it had only moved one hop down.
     Follow the `npm run` chain instead, so regrouping the gate cannot make this lie. */
  const reachable = (from) => {
    const seen = new Set();
    const walk = (name) => {
      if (seen.has(name) || !pkg.scripts[name]) return;
      seen.add(name);
      for (const m of pkg.scripts[name].matchAll(/npm run ([\w:-]+)/g)) walk(m[1]);
    };
    walk(from);
    return seen;
  };
  assert.ok(reachable('test').has('test:visual'),
    'the atlas check is not reachable from `npm test`');
});
