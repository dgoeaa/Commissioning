...
test.describe('archive — closure evidence', () => {
  ...
  test('a reference the register does not hold cannot be archived', async ({ page }) => {
    ...
    /* Archiving now asks first (see "archive asks before it closes a reference for good"
       below), so the refusal this test is about only happens once the officer confirms.
       Answering the dialog is what puts the original assertion back in front of the guard
       rather than in front of the new prompt. */
    await confirmYes(page);
    ...
    /* This used to assert on `.first()` of TWO error toasts — executeOwnedAction toasted the
       failure and archive's own catch toasted it again — and called the duplication cosmetic.
       It is not cosmetic and it is now fixed, so the count is part of the assertion: the
       governance layer stays quiet here (`notify:false`) because this screen can name the
       reference and distinguish the two refusal reasons, which the governance layer cannot.
       Toasts live 4200ms, so a count taken now sees everything that was raised. */
    const toasts = page.locator('.dgo-toast--error');
    await expect(toasts.first()).toBeVisible({ timeout: 3_000 });
    await expect(toasts, 'one failure must produce exactly one toast').toHaveCount(1);
    await expect(toasts.first(), 'the refusal must say the reference is unknown, not that work is outstanding')
      .toContainText('NITDA-2026-999999');
    /* And it must say it in the operator's language. The governance layer used to put
       `error.message` on screen; the raw text is still recorded in the audit trail. */
    const shown = await toasts.first().textContent();
    expect(shown, 'the refusal must not use the vocabulary reserved for Diagnostics')
      .not.toMatch(/\b(endpoint|payload|backend|runtime|posture)\b/i);
  });

  /* Archiving was the last consequential mutating action with no confirmation. canClose()
     refuses anything with open work, but "Inspect" fills the reference straight into the
     form — so closing a record permanently was two clicks with nothing in between. And it
     is not reversible: reopenArchive() mints a NEW derived reference and leaves the
     original archived. These lock the prompt in. */
  /* The reference has to be created AFTER the workspace mounts: mount() calls
     hydrateGovernance(), which rehydrates the entity store from State and clears whatever
     was in it first. Seeding before the route opens produces a reference that is gone by
     the time the form reads it — and a test that then passes for the wrong reason. */
  const openArchiveWithClosableRef = async page => {
    await open(page, 'archive');
    return page.evaluate(async () => {
      const { Entities } = await import('./core/entity-store.js');
      const ref = 'NITDA-2026-000910';
      Entities.createReference({ __ref: ref, __status: 'arrival' });
      return ref;
    });
  };

  test('archive asks before it closes a reference for good', async ({ page }) => {
    await boot(page);
    const ref = await openArchiveWithClosableRef(page);

    await page.fill('#archive-form [name="ref"]', ref);
    await page.click('[data-archive]');

    const dlg = page.locator('[data-dialog="confirm"]').last();
    await expect(dlg).toBeVisible({ timeout: 5_000 });
    // The dialog has to name the record — a generic "are you sure?" on a permanent act is
    // not a decision an officer can make.
    await expect(dlg).toContainText(ref);
    await expect(dlg, 'the officer must be told the closure cannot be undone')
      .toContainText(/permanent/i);
    // reopenArchive() derives a new reference; it does not restore this one. An officer who
    // believes archiving is reversible because "you can reopen it" has been misled.
    await expect(dlg, 'reopening must be described as a new reference, not a restore')
      .toContainText(/creates a new one|new reference/i);
  });

  test('cancelling the confirmation archives nothing', async ({ page }) => {
    await boot(page);
    const ref = await openArchiveWithClosableRef(page);
    ...
    const archived = await page.evaluate(async r => {
      const { Entities } = await import('./core/entity-store.js');
      return !!Entities.getArchive(r);
    }, ref);
    expect(archived, 'declining the prompt must leave the reference open').toBe(false);
  });

  test('confirming it still closes the reference — the prompt is a gate, not a block', async ({ page }) => {
    await boot(page);
    const ref = await openArchiveWithClosableRef(page);
    ...
    await expect.poll(async () => page.evaluate(async r => {
      const { Entities } = await import('./core/entity-store.js');
      return !!Entities.getArchive(r);
    }, ref), { timeout: 8_000 }).toBe(true);
    expect((await auditFor(page, 'archive-reference')).length).toBeGreaterThan(0);
  });
...
test.describe('email channel (channel B)', () => {
  ...
  test('saving a composed email holds it as a draft and does not send it', async ({ page }) => {
    ...
    /* The literal this used to match ("No sent email correspondences yet") was replaced by the
       I-13 empty-state contract, which distinguishes "nothing sent yet" from "could not load"
       and from "filtered to nothing". Asserting the contract's noun rather than one of its four
       sentences keeps the property — the register is empty and the draft is not in it — without
       pinning the test to copy the redesign deliberately rewrote. */
    const register = await page.textContent('[data-outlet]');
    expect(register, 'the register must render the empty-state contract for sent emails')
      .toMatch(/sent emails/i);
    expect(register, 'a draft must never be listed in the record of what was sent')
      .not.toContain(REF);
  });
